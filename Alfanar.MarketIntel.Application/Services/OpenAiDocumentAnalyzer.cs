...
{truncatedText}";
    ...
    {
        if (!IsAvailable())
            return Result<IntelligenceReportJsonDto>.Failure("OpenAI service not configured");
        ...
        try
        {
            ...
            var result = await CallOpenAiAsync(prompt, 2000);
            ...
            _logger.LogInformation("DEBUG: Raw OpenAI Response ({Length} chars):\n{Content}", 
    ...
    {
        if (!IsAvailable())
            return Result<CuratedItemInsightDto>.Failure("OpenAI service not configured");
...
{truncatedText}";
            ...
            var result = await CallOpenAiAsync(prompt, 800);
            if (!result.IsSuccess)
    ...
    {
        if (!IsAvailable())
            return Result<CompetitorDetectionResultDto>.Failure("OpenAI service not configured");
...
{truncatedText}";
            ...
            var result = await CallOpenAiAsync(prompt, 900);
            if (!result.IsSuccess)
    ...
    {
        if (!IsAvailable())
            return Result<AlertConfirmationDto>.Failure("OpenAI service not configured");
...
{truncatedText}";
            ...
            var result = await CallOpenAiAsync(fullPrompt, 700);
            if (!result.IsSuccess)
